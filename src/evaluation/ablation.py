...
def run_ablation(project) -> pd.DataFrame:
    ...
    agg = (df.groupby(["axis", "value", "method"])[METRIC_COLUMNS]
             .agg(["mean", "std"]))
    agg.columns = ["_".join(c) for c in agg.columns]
    agg = agg.reset_index()
    agg.to_csv(project.table_dir / "ablation_summary.csv", index=False)

    per_scen = (df.groupby(["axis", "value", "method", "scenario"])[KEY_METRICS]
                  .agg(["mean", "std"]))
    per_scen.columns = ["_".join(c) for c in per_scen.columns]
    per_scen.reset_index().to_csv(
        project.table_dir / "ablation_per_scenario.csv", index=False)
