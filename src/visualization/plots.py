...
def _plot_nees_partitions(t: np.ndarray, pred: Optional[dict], fig_dir: Path) -> None:
    ...
    for ax, (key, dof, title) in zip(axes, available):
        ...
        # A diverged filter produces a NEES that is enormous but still *finite*
        # (~1e154), so np.isfinite alone lets it into the trace and blows the
        # y-axis up to ~1e154 while the caption claims diverged arcs are
        # excluded. The 97.5% chi-square bound for 24 DoF is ~39, so any NEES
        # above CAP is a divergence, not an inconsistency; drop it from both the
        # plotted trace and the reported mean / coverage.
        CAP = 1e4
        good = np.isfinite(v) & (np.abs(v) <= CAP)
        vplot = np.where(good, v, np.nan)  # break the line across diverged arcs
        ...
        if finite.size:
            ...
            extra = f", {n_div} diverged excl." if n_div else ""
            ax.set_title(f"{title} — mean {finite.mean():.1f}, {inside:.0%} within bounds{extra}",
                         fontsize=10)
        ax.set_ylabel("NEES")
        ax.grid(True, alpha=0.3)
...
def _plot_paired_difference(per_trial: pd.DataFrame, fig_dir: Path,
    ...
    fig, ax = plt.subplots(figsize=(11, 4.5))
    ax.boxplot([diff[m].dropna().values for m in order], labels=order, showfliers=False)
...
def _plot_storm_drivers(pred: dict, fig_dir: Path, pairing: str,
                        epoch_label: str = "2015-03-17 00:00:00 UTC") -> None:
    ...
    ax1.set_xlabel(f"Hours since {epoch_label}")
    ...
    ax1.set_title(f"Storm drivers and density — {epoch_label} ({pairing})")
    _safe_save(fig, fig_dir / f"storm_drivers_{pairing}.pdf")


def _plot_storm_error_time(preds: Dict[str, dict], fig_dir: Path, pairing: str,
                           epoch_label: str = "2015-03-17 00:00:00 UTC") -> None:
    ...
    ax.set_xlabel(f"Hours since {epoch_label}")
    ax.set_ylabel("Position error [km]")
    ax.set_yscale("log")
...
def make_storm_plots(project) -> None:
    ...
    # Label the time axis / titles with the actual storm epoch so the June and
    # December figures are not mislabelled with the March date.
    scfg = project.config.get("storm_experiment", {})
    epoch_label = (str(scfg.get("start_utc", "2015-03-17T00:00:00Z"))
    ...
    for pairing_dir in progress(pairing_dirs, desc="Storm figures", unit="pairing"):
        ...
        if not preds:
            continue
        _plot_storm_error_time(preds, fig_dir, pairing, epoch_label)
        first = next(iter(preds.values()))
        _plot_storm_drivers(first, fig_dir, pairing, epoch_label)
